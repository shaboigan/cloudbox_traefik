...
PRESET=/opt/scripts/handbrake/presets
HANDBRAKE=/opt/handbrake
UPPER=2200000
LOWER=2050000
CRF=26.0
COUNTER=0
...
for FILE in "$INPUT"/*; do
    ...
    rm $HANDBRAKE/ghb/presets.json && cp $PRESET/"$CRF.json" $HANDBRAKE/ghb/presets.json
    mv $INPUT/"$NAME" $WATCH/
    ...
    while [ "$BITRATE" -lt "$LOWER" ]; do
        if [ 1 -eq "$(echo "${CRF} > 26" | bc)" ] ;then
            mv "$OUTPUT"/"$filename".mp4 $ERROR && mv $WATCH/"$NAME" $ERROR
            break
        else
            if [ "$COUNTER" -lt 4 ]; then
                COUNTER=$(( $COUNTER + 1 ))
                mv $WATCH/"$NAME" $INPUT/"$NAME" && rm "$OUTPUT"/"$filename".mp4
                head -n -1 $HANDBRAKE/successful_conversions > $HANDBRAKE/successful_conversions.tmp ; mv $HANDBRAKE/successful_conversions.tmp $HANDBRAKE/successful_conversions
            	...
            	mv $WATCH/"$NAME" $ERROR && rm "$OUTPUT"/"$filename".mp4
                break
            fi
        fi
    done
    ...
    if [ -e "$OUTPUT"/"$filename".mp4 ]; then
        ...
        mv "$OUTPUT"/"$filename".mp4 "$RADARR" && rm $WATCH/"$NAME"
